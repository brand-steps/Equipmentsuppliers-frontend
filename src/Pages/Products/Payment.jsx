...
const handlepayment = () => {
  ...
  axios.post(`https://sore-cyan-fly-kit.cyclic.app/create-checkout-sessions`, {
    innerArray: data,
  }).then((res) => {
    if (res.data.url) {
  ...
  const addOrder = async () => {
    if (email && username && cardnumber && cardexp && address && state && zip && shipping) {
      ...
      try {
        const response = await axios.post(`https://sore-cyan-fly-kit.cyclic.app/addorders`, {
          email,
          username,
          phone,
          cardnumber,
...
<div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
  <div className="px-4 pt-8">
    ...
    <p className="text-xl font-medium">{t("order")}</p>
    <p className="text-gray-400">{t("orderdes")}</p>
...
{/*}        <i className="fa-solid fa-xmark text-left"></i>
   */}     </div>
   ...
   {/*}     <div className="flex w-full flex-col px-4 py-4">
    ...
    <p className="mt-8 text-lg font-medium">{t("shipping")}</p>
  ...
  <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
    ...
    <p className="text-gray-400">{t("paymentdes")}</p>
    <div className="">
      ...
      <label for="card-holder" className="mt-4 mb-2 block text-sm font-medium">{t("cardname")}</label>
      <div className="relative">
        <input type="text" id="card-holder" name="card-holder" onChange={(event) => { setusername(event.target.value) }} className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm uppercase shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder={`${t("cardhol")}`} />
        <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <label for="card-holder" className="mt-4 mb-2 block text-sm font-medium">{t("mobile")}</label>
      <div className="relative">
        <input type="number" id="card-holder" name="card-holder" onChange={(event) => { setphone(event.target.value) }} className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm uppercase shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder={`${t("mobilehol")}`} />
        <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z" />
          </svg>
      ...
      <label for="card-no" className="mt-4 mb-2 block text-sm font-medium">{t("carddetails")}</label>
      <div className="flex">
      ...
      <label for="billing-address" className="mt-4 mb-2 block text-sm font-medium">{t("billing")}</label>
      <div className="flex flex-col sm:flex-row">
        <div className="relative flex-shrink-0 sm:w-7/12">
          <input type="text" id="billing-address" onChange={(event) => { setaddress(event.target.value) }} name="billing-address" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder={`${t("street")}`} />
          <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
      ...
      <div className="mt-6 border-t border-b py-2">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-gray-900">{t("shippingprice")}</p>
    ...
    <button onClick={() => {addOrder(); handlepayment();}} className="mt-4 mb-8 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white">{t("placeorder")}</button>
  </div>
</div>
